...
// documentação da api em https://documenter.getpostman.com/view/18385621/UVe9SpjC
...
app.get('/tasks', (req: Request, res: Response) => {
    ...
    if (!userId || userId === null || userId === undefined) { res.status(400).send('Erro, id não informado') }
...
app.get('/tasks/:status', (req: Request, res: Response) => {
    ...
    if (req.params.status !== 'true' && req.params.status !== 'false') { res.status(400).send('Erro, status informado inválido. Deve ser true ou false apenas') }
